...
@section('admin')
    <div class="row p-2 ">
        <div class="col-md-6  bg-white p-4 rounded-3 shadow">
            <p class="f-16 font-S color-b-700 text-end">صفحه ایتم ها</p>
            <form action="{{route('admin.new.item')}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    <label for="formFilte"  class="form-label f-13 color-b-500 d-block text-end">موضوع</label>
                    <input dir="rtl"  name="title" class="form-control text-end" type="text" id="formFilte">
                </div>
                <div class="mb-3">
                    <label for="formFilte"  class="form-label f-13 color-b-500 d-block text-end">توضیحات</label>
                    <input  dir="rtl" name="text" class="form-control text-end" type="text" id="formFilte">
                </div>
                <div class="mb-3">
                        ...
                        <li class="w-100 p-3 pointer border rounded-3 mt-4">
                            <div class="form-check pointer">
                                ...
                                <label dir="rtl" class="form-check-label f-14 color-b-600 pointer w-100 text-end" for="flexRadioDefault{{$a->id}}">
                                    <img src="{{url('image/item/'.$a->icon)}}" alt="{{$a->title}}" style="width: 60px"> - <h4 class="color-b-700 text-end">{{$a->title}}</h4> - {{$a->text}}
                                </label>
                            </div>
                        </li>
                    @endforeach
    ...
    <div class="page-new page-new-delete-banner-center-as overflow-hidden">
        ...
        <button @click="delete_image_center('item')" type="button" class="btn btn-lg btn-danger f-13 ms-3 mt-3">
            بله
        </button>
