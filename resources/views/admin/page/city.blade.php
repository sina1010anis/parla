...
@section('admin')
    <div class="row p-2 ">
        <div class="col-md-6  bg-white p-4 rounded-3 shadow">
            ...
            <form action="{{route('admin.new.city')}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                ...
                <div class="mb-3">
                    <label for="formFilte" class="form-label f-13 color-b-500 d-block text-end">استان</label>
        ...
        <div class="col-md-6  mt-3 bg-white p-4 rounded-3 shadow text-center">
            <div class=" overflow-scroll rounded-3 position-relative bg-white" style="max-height: 300px">
                <select v-model="id_menu_admin" class="form-select" aria-label="Default select example">
                    @foreach($state as $s)
                        <option value="{{$s->id}}">{{$s->name}}</option>
                    @endforeach
                </select>
                <ul class="m-0 p-0">
                    @foreach($city as $a)
                        <li v-if="{{$a->city_id}} == id_menu_admin" class="w-100 p-3 pointer border rounded-3 mt-4">
                            <div class="form-check pointer">
                                <i @click="view_page_delete('{{$a->id}}')" class="bi bi-trash f-17 position-absolute"
                                   style="color:red;"></i>
                                <label dir="rtl" class="form-check-label f-14 color-b-600 pointer w-100 text-end"
                                       for="flexRadioDefault{{$a->id}}">
    ...
    <div class="page-new page-new-delete-banner-center-as overflow-hidden">
        ...
        <button @click="delete_image_center('city')" type="button" class="btn btn-lg btn-danger f-13 ms-3 mt-3">
            بله
        </button>
