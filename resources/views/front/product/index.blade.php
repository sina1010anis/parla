...
@section('index')
    ...
    <view-product :data="{{$data}}" :image="{{$data->image_product}}">
        ...
        <template #btn_send_card>
            <a :class="(status_card.step_1 == 0 || status_card.step_2 == 0) ? 'btn btn btn-red font-Y f-14 float-end btn-send-card disabled' : 'btn btn btn-red font-Y f-14 float-end btn-send-card '" type="button">خرید</a>
        </template>
        ...
        <template #comment_product>
            @foreach($data->comments as $comment)
                <div class=" w-100 bg-white shadow-sm rounded-3 mt-3 overflow-scroll mt-0 color-b-600 f-13 line-h-30 text-end p-2" dir="rtl">
                <div class="col-12 p-2">
                    <span><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class=" bi bi-person-circle" viewBox="0 0 16 16"><path d="M11 6a3 3 0 1 1-6 0 3 3 0 0 1 6 0z"></path><path fill-rule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm8-7a7 7 0 0 0-5.468 11.37C3.242 11.226 4.805 10 8 10s4.757 1.225 5.468 2.37A7 7 0 0 0 8 1z"></path></svg><span class="position-relative name-user-comment">{{$comment->user->name}}</span></span><span class="float-start f-10 color-b-400">{{jdate($comment->created_at)->format('%B %d، %Y')}}</span>
                </div>
                            ...
                            <div class="row mt-2">
                                ...
                                <div class="col-9 ">
                                    <div class="progress position-relative" style="top: 8px">
                                        <div class="progress-bar bg-gh text-dark  f-9" role="progressbar" style="width: {{$comment->designing}}%" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">{{$comment->designing}}%</div>
                                    </div>
                                </div>
                            </div>
                            <div class="row mt-2">
                                ...
                                <div class="col-9 ">
                                    <div class="progress position-relative" style="top: 8px">
                                        <div class="progress-bar bg-gh text-dark f-9" role="progressbar" style="width: {{$comment->possibilities}}%" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">{{$comment->possibilities}}%</div>
                                    </div>
                                </div>
                            </div>
                            <div class="row mt-2">
                                ...
                                <div class="col-9 ">
                                    <div class="progress position-relative" style="top: 8px">
                                        <div class="progress-bar bg-gh text-dark f-9" role="progressbar" style="width: {{$comment->value}}%" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">{{$comment->value}}%</div>
                                    </div>
                                </div>
                            </div>
                            <div class="row mt-2">
                                ...
                                <div class="col-9 ">
                                    <div class="progress position-relative" style="top: 8px">
                                        <div class="progress-bar bg-gh text-dark f-9" role="progressbar" style="width: {{$comment->quality}}%" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">{{$comment->quality}}%</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                @if($comment->src_image_user)
                    <div class="col-12 p-3 obj-center">
                        <img src="/image/product/{{$comment->src_image_user}}" style="max-width: 100%;width: 200px" alt="">
                    </div>
                @endif
            </div>
            @endforeach
        ...
        <template #color_product>
            @foreach($data->color as $color)
                <li @click="set_color({{$color->id}} , '{{$color->color->name}}')" title="test" class="obj-center text-center p-2 color-b-600 m-2 d-inline-block" style="background-image: url('/image/color/{{$color->color->code}}')">
                    <span class="f-11 color-b-100 float-start"> {{$color->color->name}}</span>
                </li>
            @endforeach
        </template>
        <template #size_all>
            <select @change="select_size" v-model="id_size_product" class="form-select  mb-3" id="select_size" aria-label=".form-select-lg example">
        ...
        <template #price_product>
            @if($data->discount <= 0)
                <p dir="rtl" align="right" class="f-14" style="color: #ff5454">قیمت: <b class="f-20 color-b-900">@{{ (data_size != null) ?data_size.price : 'لطفا یک سایز انتخاب کنید' }}</b></p>
            @else
        ...
        <template #form_comment>
            <form action="">
                ...
                <div class="form-floating">
                    <textarea dir="rtl" class="form-control" align="right" placeholder="متن کامنت" id="floatingTextarea"></textarea>
                    <label class="color-b-500 text-end" dir="rtl" for="floatingTextarea">متن پیام</label>
                </div>
                <div class="row my-3">
                    <div class="col-6">
                        <div class="form-floating">
                            <select class="form-select f-12 text-center" id="floatingSelectGrid" aria-label="Floating label select example">
                    ...
                    <div class="col-6 my-2">
                        <div class="form-floating">
                            <select class="form-select form-select-sm f-12 text-center" id="floatingSelectGrid" aria-label="Floating label select example">
                                <option>خیلی کم</option>
                                <option value="1">کم</option>
                                <option value="2">خوب</option>
                                <option value="3">خیلی خوب</option>
                    ...
                    <div class="my-3">
                        <label for="formFile"  class="form-label color-b-500 f-11" dir="rtl">اگر محصول را خریداری کرده اید لطفا عکس محصول را هم اپلود کنید .</label>
